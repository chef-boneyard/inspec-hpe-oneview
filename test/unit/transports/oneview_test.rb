...
describe 'oneview transport' do
  ...
  describe 'options' do
    it 'defaults to env options' do
      ...
      options[:oneview_name].must_equal 'oneview_resource_name'
      options[:oneview_type].must_equal 'oneview_resource_type'
    end
  end

  it 'allows for options override' do
    transport = transport(oneview_settings_file: settings_file_override.path, oneview_name: 'override_name', oneview_type: 'override_type')
    ...
    options[:oneview_name].must_equal 'override_name'
    options[:oneview_type].must_equal 'override_type'
  end
  ...
  describe 'platform' do
    it 'returns platform' do
      ...
      platform.family_hierarchy.must_equal %w[iaas api]
    end
  end
  ...
  describe 'resources' do
    it 'raises an error with nil arguments' do
      options[:oneview_name] = nil
      options[:oneview_type] = nil
      connection.connect
  ...
  describe 'connect' do
    let(:connect_settings) do
      require 'tempfile'
      file = Tempfile.new('connect_settings')
      info = <<~INFO
        {
